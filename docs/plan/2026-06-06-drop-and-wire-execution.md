...
- **D1 Phase B(连线→数组参考)做不做?** 它价值低(连线是快捷方式)、风险高(碰 `connectToNode` 边语义 + M6 数组不画边)。
  建议:**本期只做 Phase A(拖文件到节点)**,Phase B 单独评估;单帧连线现状已工作不动。——除非你要求一起做。
- **D2 拖悬停态要不要先出 HTML 样张?** 建议:状态简单,直接按 §4 token 实现 + design-fidelity 断言守;你要样张我就补 v4.1。
