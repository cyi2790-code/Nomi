...
import {
    getAgentCreatableGenerationNodeKinds,
...
function buildGenerationCanvasAgentPrompt(
...
async function defaultExecuteToolCall(event: ToolCallEvent): Promise<void> {
    ...
    try {
        ...
        if (toolName === "create_canvas_nodes") {
            ...
            const inputs: CreateGenerationNodeToolInput[] = (
                payload.nodes || []
            ).map((node, index) => ({
                kind: node.kind,
                title:
                    node.title ||
                    `${getGenerationNodeDefaultTitle(node.kind)} ${index + 1}`,
                prompt: node.prompt || "",
            ...
            (payload.nodes || []).forEach((node, index) => {
                if (node.clientId && created[index])
                    clientIdToNodeId[node.clientId] = created[index].id;
            });
            await confirm({
                ok: true,
                result: {
        ...
        if (toolName === "connect_canvas_edges") {
            ...
            await confirm({
                ok: true,
                result: { connectedCount: edges.length },
            });
            return;
        }
        if (toolName === "set_node_prompt") {
            const payload = args as { nodeId: string; prompt: string };
            const node = generationCanvasTools.update_node_prompt(
                payload.nodeId,
                payload.prompt,
            );
            await confirm({
        ...
        if (toolName === "delete_canvas_nodes") {
            ...
            await confirm({
                ok: false,
                message: "delete_canvas_nodes is not yet implemented",
            });
            return;
        }
            ...
            if (event.event === "tool-call") {
                ...
                const toolCallEvent: ToolCallEvent = {
                    ...
                    confirm: async (decision) => {
                        ...
                        await activeSession.confirmTool(
                            data.toolCallId,
                            decision,
                        );
                    },
